...
c_private_key = random.getrandbits(256)

# Paso 3: Simular el intercambio de números entre Alice y Bob

A = pow(g, a_private_key, p)
B = pow(g, b_private_key, p)
C = pow(g, c_private_key, p)

# Paso 4: Calcular la llave secreta "s" y verificar que sean iguales mediante SHA-256
s_alice_eve= pow(C, a_private_key, p)
s_eve_alice = pow (A, c_private_key, p)
s_bob_eve = pow(C, b_private_key, p)
s_eve_bob = pow(B, c_private_key, p)
